  ...
  return (
    <div style={{ fontFamily: 'var(--mono)', fontSize: 12.5, lineHeight: 1.85 }}>
      {lines.map((line, i) => {
        ...
        if (ASMP.test(t)) return <div key={i} style={{ background:'var(--warn-light)', border:'1.5px solid rgba(217,119,6,0.3)', borderRadius:7, padding:'9px 13px', marginBottom:12, fontSize:11, color:'var(--warn)', fontWeight:600 }}>{t}</div>
        if (MAIN.test(t)) return <div key={i} style={{ fontSize:10, fontWeight:800, letterSpacing:2.5, color:'var(--accent)', marginTop:22, marginBottom:8, paddingBottom:7, borderBottom:'2px solid var(--accent-light)', textTransform:'uppercase' }}>{t}</div>
        const sub = t.match(SUB)
        if (sub) return <div key={i} style={{ marginTop:10, marginBottom:2 }}><span style={{ fontWeight:700, color:'var(--text)', fontSize:11 }}>{sub[1]}:</span><span style={{ color:'var(--text-mid)' }}> {sub[2]}</span></div>
        if (NUM.test(t)) return <div key={i} style={{ color:'var(--text)', fontWeight:600, marginBottom:4 }}>{t}</div>
      ...
      {queries.length > 0 && (
        <div style={{ marginTop:24, paddingTop:16, borderTop:'2px solid rgba(217,119,6,0.2)' }}>
          <div style={{ fontSize:10, fontWeight:800, letterSpacing:2, color:'var(--warn)', marginBottom:12, textTransform:'uppercase' }}>Queries Requiring Response</div>
          {queries.map((q,i) => (
...
export default function Home() {
  ...
  // Speech recognition (browser-based, instant, no API needed)
  ...
  useEffect(() => {
    try {
      ...
      if (sess) setSessions(JSON.parse(sess).filter(s => s.expires > Date.now()))
    } catch(e) {}
  }, [])

  const startWave = () => {
    waveRef.current = setInterval(() => setWaveHeights(Array(52).fill(0).map(() => Math.random()*38+4)), 80)
  }
  const stopWave = () => { clearInterval(waveRef.current); setWaveHeights(Array(52).fill(3)) }

  const startRecording = () => {
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition
    if (!SR) {
      setStatus('error')
      setStatusMsg('Please use Google Chrome for voice dictation')
      return
    }
    ...
    recognition.onerror = (e) => {
      ...
      if (e.error === 'not-allowed') {
        setStatus('error')
        setStatusMsg('Microphone blocked — click the lock icon in Chrome address bar and allow microphone')
      }
    }
    recognition.onend = () => {
    ...
    try { recognition.start() } catch(e) {
      setStatus('error')
      setStatusMsg('Could not start microphone — please allow access in Chrome')
    }
  }
  ...
  const stopAndSend = () => {
    ...
    if (!text) { setStatus('error'); setStatusMsg('No speech detected — please try again'); return }
    setTranscript(text)
    formatReport(text)
  }
  ...
  const formatReport = useCallback(async (transcriptText, isFollowUp=false, followUpData=null) => {
    ...
    try {
      ...
      } : { transcript: transcriptText, preferences, styleProfile }
      ...
      if (data.hasQueries) {
        setStatus('queries')
        setStatusMsg(`${data.queries.length} ${data.queries.length===1?'query':'queries'} — please answer below`)
      } else {
        setStatus('done')
  ...
  const submitQueryAnswers = async () => {
    ...
    setQueryAnswers('')
    await formatReport(transcript, true, { originalReport: report, queries, queryAnswers })
  }

  const autoSave = (reportText) => {
    const session = { id: Date.now(), label: template||'Report', report: reportText, transcript, created: new Date().toISOString(), expires: Date.now()+86400000 }
    setSessions(prev => {
  ...
  const loadSession = (session) => {
    setReport(session.report); setTranscript(session.transcript||''); setQueries([])
    setStatus('done'); setStatusMsg('Session loaded'); setPipelineStep(4)
    setActiveSession(session.id); setActiveTab('report')
  }

  const deleteSession = (id) => {
    setSessions(prev => { const u=prev.filter(s=>s.id!==id); try{localStorage.setItem('ms_sessions',JSON.stringify(u))}catch(e){}; return u })
  ...
  const savePref = () => {
    ...
    setPreferences(updated); setNewPref('')
  ...
  return (
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', overflow:'hidden', background:'var(--bg)' }}>
      ...
      {/* HEADER */}
      <header style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)', padding:'0 24px', height:58, display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0, boxShadow:'0 1px 3px rgba(0,0,0,0.04)', zIndex:10 }}>
        <div style={{ display:'flex', alignItems:'center', gap:14 }}>
          <span style={{ fontFamily:'var(--serif)', fontStyle:'italic', fontSize:24, color:'var(--accent)', fontWeight:700, letterSpacing:-0.5 }}>Miranda Scribe</span>
          <div style={{ width:1, height:20, background:'var(--border)' }} />
          <span style={{ fontSize:10, fontWeight:700, color:'var(--text-dim)', letterSpacing:2, textTransform:'uppercase' }}>AI Radiology Dictation</span>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          {statusMsg && (
            <div style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 13px', borderRadius:20, fontSize:11, fontWeight:600, background:statusStyle.bg, color:statusStyle.color, border:`1px solid ${statusStyle.border}`, maxWidth:360, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
              {(status==='formatting') && <span style={{ width:10, height:10, border:'2px solid currentColor', borderTopColor:'transparent', borderRadius:'50%', display:'inline-block', animation:'spin 0.7s linear infinite', opacity:0.6, flexShrink:0 }} />}
              {status==='recording' && <span style={{ width:6, height:6, borderRadius:'50%', background:'var(--danger)', display:'inline-block', animation:'pulse 1s infinite', flexShrink:0 }} />}
              {statusMsg}
            </div>
          )}
          <button onClick={()=>setDark(d=>!d)} style={{ width:36, height:36, borderRadius:8, background:'var(--surface2)', border:'1.5px solid var(--border)', color:'var(--text-mid)', fontSize:16, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}>
            {dark?'☀':'🌙'}
          </button>
        </div>
      </header>
        ...
        <div style={{ background:'var(--surface)', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', overflow:'hidden' }}>
          <div style={{ padding:'14px 12px 10px', borderBottom:'1px solid var(--border)' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:5 }}>
              {[['MRI','🧲'],['CT','⚙️'],['US','📡'],['XR','🩻']].map(([m,icon]) => (
                <button key={m} onClick={()=>{setModality(m);setTemplate(TEMPLATES[m][0].name)}}
                  style={{ padding:'9px 4px', borderRadius:8, border:'1.5px solid', borderColor:modality===m?'var(--accent)':'var(--border)', background:modality===m?'var(--accent)':'var(--surface2)', color:modality===m?'#fff':'var(--text-mid)', fontSize:12, fontWeight:700, cursor:'pointer', transition:'all 0.15s', display:'flex', flexDirection:'column', alignItems:'center', gap:3 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', overflow:'hidden', background:'var(--bg)' }}>
          <div style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)', padding:'22px 24px 18px', flexShrink:0, boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:20 }}>
              <div style={{ flex:1 }}>
                <div style={{ fontFamily:'var(--serif)', fontStyle:'italic', fontSize:22, color:'var(--text)', marginBottom:4 }}>Dictation Studio</div>
              ...
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8, flexShrink:0 }}>
                <button onClick={isRecording ? stopAndSend : startRecording}
                  ...
                  style={{ width:88, height:88, borderRadius:'50%', background:isRecording?'var(--danger)':'var(--surface)', border:`3px solid var(--danger)`, color:isRecording?'#fff':'var(--danger)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:2, cursor:status==='formatting'?'not-allowed':'pointer', opacity:status==='formatting'?0.5:1, transition:'all 0.2s', animation:isRecording?'recordPulse 1.2s infinite':'none', boxShadow:isRecording?'':'0 4px 18px rgba(220,38,38,0.15)' }}>
                  <span style={{ fontSize:28, lineHeight:1 }}>{isRecording?'⏹':'🎙'}</span>
                  <span style={{ fontSize:8, fontWeight:800, letterSpacing:1.5, textTransform:'uppercase' }}>{isRecording?'Stop & Send':'Record'}</span>
                </button>
                {isRecording && <div style={{ fontFamily:'var(--mono)', fontSize:13, color:'var(--danger)', fontWeight:600 }}>{fmtTime(recordSecs)}</div>}
              </div>
            </div>
            ...
            <div style={{ marginTop:14, height:50, background:'var(--surface2)', border:'1.5px solid var(--border)', borderRadius:10, display:'flex', alignItems:'center', padding:'0 14px', gap:2, overflow:'hidden', position:'relative' }}>
              {isRecording ? (
                waveHeights.map((h,i) => <div key={i} style={{ width:3, height:h, background:'var(--accent)', borderRadius:2, opacity:0.7, flexShrink:0, transition:'height 0.08s ease' }} />)
              ) : (
                <span style={{ position:'absolute', left:'50%', transform:'translateX(-50%)', fontSize:11, color:'var(--text-dim)', fontWeight:500, whiteSpace:'nowrap' }}>
                  {status==='formatting'?'⚡ Claude is formatting your report...':'Waveform appears during recording'}
                </span>
              )}
            </div>
          </div>
          ...
          <div style={{ flex:1, overflow:'auto', padding:'18px 22px', display:'flex', flexDirection:'column', gap:12 }}>
            {!transcript ? (
              <div style={{ textAlign:'center', padding:'52px 20px', color:'var(--text-dim)' }}>
                ...
                <div style={{ fontSize:12, lineHeight:2 }}>
                  Press Record and speak naturally.<br/>
                  Transcription happens instantly in Chrome.<br/>
                  Claude formats the complete report automatically.
                </div>
              </div>
            ) : (
              <div>
                <div style={{ fontSize:9, fontWeight:700, letterSpacing:2, color:'var(--text-dim)', textTransform:'uppercase', marginBottom:8, display:'flex', alignItems:'center', gap:8 }}>
                  Raw Transcript <div style={{ flex:1, height:1, background:'var(--border)' }} />
                </div>
                ...
                {(status==='idle'||status==='error'||status==='done') && transcript && (
                  <button onClick={()=>formatReport(transcript)} style={{ marginTop:10, width:'100%', padding:'13px', background:'var(--accent)', border:'none', borderRadius:10, color:'#fff', fontSize:14, fontWeight:700, boxShadow:'0 4px 14px rgba(26,110,245,0.3)', cursor:'pointer' }}>
                    ✦ Re-format Report
                  </button>
                )}
              </div>
          ...
          <div style={{ padding:'0 16px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', gap:2, height:48, flexShrink:0 }}>
            {[['report','Report'],['history',`History (${sessions.length})`],['prefs','Preferences']].map(([id,label]) => (
              <button key={id} onClick={()=>setActiveTab(id)}
                style={{ padding:'6px 14px', borderRadius:7, fontSize:12, fontWeight:600, border:'1.5px solid', borderColor:activeTab===id?'rgba(26,110,245,0.2)':'transparent', background:activeTab===id?'var(--accent-light)':'transparent', color:activeTab===id?'var(--accent)':'var(--text-dim)', cursor:'pointer', transition:'all 0.12s' }}>
                {label}
              </button>
            ))}
          </div>
          ...
          {activeTab==='report' && (
            <>
              ...
              {status==='queries' && queries.length>0 && (
                <div style={{ margin:'0 14px 12px', padding:'14px', background:'var(--warn-light)', border:'2px solid rgba(217,119,6,0.2)', borderRadius:10, flexShrink:0 }}>
                  ...
                  <textarea value={queryAnswers} onChange={e=>setQueryAnswers(e.target.value)}
                  ...
                  <button onClick={submitQueryAnswers} style={{ width:'100%', marginTop:8, padding:11, background:'var(--accent)', border:'none', borderRadius:8, color:'#fff', fontSize:13, fontWeight:700, cursor:'pointer' }}>
              ...
              {report && (
                <div style={{ padding:'12px 14px', borderTop:'1px solid var(--border)', display:'flex', gap:8, flexShrink:0 }}>
                  <button onClick={copyReport} style={{ flex:1, padding:'11px', borderRadius:9, fontSize:13, fontWeight:700, background:copied?'var(--teal)':'var(--accent)', border:'none', color:'#fff', boxShadow:`0 3px 10px ${copied?'rgba(13,148,136,0.25)':'rgba(26,110,245,0.25)'}`, cursor:'pointer', transition:'all 0.2s' }}>
                    {copied?'✓ Copied!':'📋 Copy — RIS Ready'}
                  </button>
                  <button onClick={clearAll} style={{ padding:'11px 14px', borderRadius:9, fontSize:12, fontWeight:600, background:'var(--surface2)', border:'1.5px solid var(--border)', color:'var(--text-dim)', cursor:'pointer' }}>Clear</button>
                </div>
              )}
            </>
          )}
              ...
              {sessions.length===0 ? (
                ...
                return (
                  <div key={s.id} onClick={()=>loadSession(s)}
                    style={{ padding:'12px 13px', border:'1.5px solid', borderColor:active?'rgba(26,110,245,0.3)':'var(--border)', borderRadius:9, marginBottom:8, background:active?'var(--accent-light)':'var(--surface)', cursor:'pointer', transition:'all 0.12s' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:5 }}>
                      <span style={{ fontSize:12, fontWeight:600, color:active?'var(--accent)':'var(--text)' }}>{s.label}</span>
                      <button onClick={e=>{e.stopPropagation();deleteSession(s.id)}} style={{ background:'none', border:'none', color:'var(--text-dim)', fontSize:14, cursor:'pointer', padding:0 }}>×</button>
                    </div>
          ...
          {activeTab==='prefs' && (
            <div style={{ flex:1, overflow:'auto', padding:'16px' }}>
              ...
              <div style={{ display:'flex', gap:7, marginBottom:10 }}>
                <input value={newPref} onChange={e=>setNewPref(e.target.value)} placeholder='e.g. Always mention bone age in paediatric reports'
                ...
                <button onClick={savePref} style={{ padding:'9px 14px', background:'var(--accent)', border:'none', borderRadius:8, color:'#fff', fontSize:12, fontWeight:700, cursor:'pointer' }}>Add</button>
              </div>
              ...
              <div style={{ fontSize:11, color:'var(--text-dim)', lineHeight:1.7, marginBottom:10 }}>Paste 3-10 of your past reports. Miranda Scribe will match your writing style.</div>
              ...
              <button onClick={saveStyle} style={{ width:'100%', padding:'10px', background:'var(--accent)', border:'none', borderRadius:8, color:'#fff', fontSize:13, fontWeight:700, cursor:'pointer' }}>Save Style Profile</button>
              <div style={{ marginTop:16, padding:'12px', background:'var(--surface2)', border:'1px solid var(--border)', borderRadius:9, fontSize:11, color:'var(--text-dim)', lineHeight:1.8 }}>
                <strong style={{ color:'var(--text-mid)' }}>Always applied:</strong><br/>
                Plain ASCII · NZ English · No smart quotes · No em dashes · No "degenerative" in conclusions · Grade 1-4 chondral damage · Full ACL documentation · Ultrasound lipoma wording · No bursopathy
              </div>
            </div>
          )}
        </div>
